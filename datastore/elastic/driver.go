...
func (ed *elasticDriver) GetConnection() (datastore.Connection, error) {
	es, err := elasticsearch.NewClient(elasticsearch.Config{Addresses: []string{
		ed.elasticURL(),
	}})
	if err != nil {
...
func (ed *elasticDriver) postMappings() error {
	//TODO mapping types are depricated since 6.x release and it's completely removed from 8.x version
	//https://www.elastic.co/guide/en/elasticsearch/reference/7.6/removal-of-types.html#_why_are_mapping_types_being_removed
	...
	plog.Infof("Mapping dict %s", ed.mappings)
...
func (ed *elasticDriver) postIndex() error {
	...
	logger.WithField("config", string(configBytes)).Info("Posting Index")
